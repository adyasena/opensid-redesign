    ...
    <div class="flex lg:flex-row flex-col justify-between items-center relative z-10">
      ...
      <div class="flex flex-col lg:flex-row gap-3 mt-5 lg:mt-0 flex-wrap lg:justify-end w-full px-3">
        <form action="<?= site_url() ?>" role="form" class="relative">
          ...
          <input type="text" name="cari" class="text-black form-input px-10 w-full bg-white relative inline-block" placeholder="Cari..." style="height:35px;">
        </form>
        <?php if($this->setting->layanan_mandiri == 1) : ?>
          <a href="<?= site_url('layanan-mandiri') ?>" class="btn btn-accent text-sm w-full lg:w-auto text-center">Layanan Mandiri <i class="fas fa-external-link-alt ml-1"></i></a>
        <?php endif ?>
      </div>
    </div>
  </div>
